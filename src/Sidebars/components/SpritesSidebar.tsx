...
const useStyles = makeStyles({
  ...
  backgroundsContainer: {height: '100%', width: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', alignItems: 'center', justifyContent: 'center', gap: 10},
})
...
export default function SpritesSidebar() {
  ...
  useEffect(() => {
    ...
    const getBg = async () => {
      ...
      const imageRefs = await list(storageRef(storage, "backgrounds"), { maxResults: 10, pageToken: currentToken});
          ...
          >
            <div className={classes.backgroundsContainer}>
              {backgrounds?.map((bg: any, index: number) => (
                <div key={`bg-image-${index}`} onClick={() => handleFrameBackground(bg)} style={{width: '100%', display: 'flex', justifyContent: 'center'}}>

  ...
  const [{ isOver }, drop] = useDrop({
    ...
    drop: (item: any, monitor) => {
      if (item.type === "SIDEBAR_SPRITE") {
        ...
        const containersWidthSpacing = Math.round(
          (canvasContainer.current?.clientWidth - innerCanvasWidth * scale) / 2
        );
        const containersHeightSpacing = Math.round(
          (canvasContainer.current?.clientHeight - innerCanvasHeight * scale) /
            2
        );
        createSprite(
          {
        ...
        const deltaX = Math.round(
          (monitor.getDifferenceFromInitialOffset()?.x || 0) / scale
        );
        const deltaY = Math.round(
          (monitor.getDifferenceFromInitialOffset()?.y || 0) / scale
        );
